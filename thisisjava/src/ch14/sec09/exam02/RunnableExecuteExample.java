...
// 관리자가 1000명의 회원에게 신상품 입고 내용의 이메일 보내기 작업
// Runnable 1000개 생성 후 execute() 메소드로 작업 큐에 넣고 ExecutorService는 최대 5개 스레드로 작업큐에서 Runnable을 하나씩 꺼내서 run() 메소드 실행

// 1000개의 메일을 최대 5개의 스레드를 만들어 메일 보내기 처리를 하고 종료하는 프로그램
...
public class RunnableExecuteExample {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<mails.length; i++) {
			mails[i][0] = "admin@ny.com";
			mails[i][1] = "member_" + i + "@my.com";
			mails[i][2] = "신상품이 입고되었습니다.";
		}
		
		//최대 5개인 스레드풀 생성
		ExecutorService es = Executors.newFixedThreadPool(5);
		
		//메일 보내는 작업 생성 및 작업큐로 처리 요청
		for(int i=0; i<mails.length; i++) {
			final int idx = i;
		...
		// 모든 스레드가 작업을 다 처리하면 스레드풀 종료 (es.shutdownNow() 스레드 풀 강제 종료하고 대기 작업 반환)
		es.shutdown();
