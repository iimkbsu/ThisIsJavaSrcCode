...
/*
 ...
 리플랙션
 	...
 	- 패키지 및 타입 정보 얻기
 		- myClass.getPackage();			: 패키지 정보 읽기 (Package 타입)
 		- myClass.getSimpleName();		: 패키지 제외한 타입 이름 (String 타입)
 		- myClass.getName();			: 패키지 포함 전체 타입 이름 (String 타입)
 		
 	
 	- 맴버(생성자, 필드, 메소드) 정보 얻기 (import java.lang.reflect.*)
 		- myClass.getDeclaredConstructors()		: 생성자 정보 읽기 (Constructor[] 타입) (.getParameterTypes()으로 매개변수 받고 printParameters()로 출력)
 		- myClass.getDeclaredFields()			: 필드 정보 읽기 (Field[] 타입) 
 		- myClass.getDeclaredMethods()			: 메소드 정보 읽기 (Method[] 타입) (.getParameterTypes()으로 매개변수 받고 printParameters()로 출력)
 	...
 	- 리소스 경로 얻기 (이미지, xml, 프로퍼티 등 리소스의 경로)
 		- myClass.getResource(String 리소스이름);					: 리소스 파일의 URL(경로) 리턴 (URL 타입)
 		- myClass.getResource(String 리소스이름).getPath();			: 리소스 파일의 URL을 문자열 경로로 리턴 (String 타입)
 		- myClass.getResourceAsStream(String 리소스이름);			: 리소스 파일의 InputStream(파일데이터)을 리턴 (InputStream 타입)
